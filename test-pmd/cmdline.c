...
/* *** Show flow LRU statistic. *** */
struct cmd_show_lru_result {
	cmdline_fixed_string_t help;
};

static void cmd_show_lru_parsed(__rte_unused void *parsed_result,
                                  struct cmdline *cl,
                                  __rte_unused void *data)
{
	printf("\n Show latest 10 flow statistics:\n");
	...
	while (entry != NULL && i < 50) {
			...
			// uint32_t l1 = 1241557184;
			// memcpy(&dst, &l1, 4);
			// memcpy(&src, &entry->key.srcip, 4);
			// dst.s_addr = entry->key.dstip;
			// src.s_addr = entry->key.srcip;
			
			// printf("flow: %lu, %s:%d -> %s:%d packet: %d, size: %d\n", entry->key.dstip, inet_ntoa(src),entry->key.srcport,inet_ntoa(dst),entry->key.dstport,entry->packet_count, cache->lruListSize);
			//printf("%x %x %x %x %x, hash: %d, %x\n", entry->key.srcip, entry->key.srcport, entry->key.dstip,entry->key.dstport, entry->key.protocol, flow_key_hash_4(entry->key), lruCache->hashMap[flow_key_hash_4(entry->key)]);
			printf("FlowID:%x %x %x %x %x; Packet_Count: %d\n", entry->key.srcip, entry->key.srcport, entry->key.dstip,
			entry->key.dstport, entry->key.protocol, entry->packet_count);
...
cmdline_parse_token_string_t cmd_show_lru_help =
	TOKEN_STRING_INITIALIZER(struct cmd_show_lru_result, help, "lru");

cmdline_parse_inst_t cmd_show_lru_brief = {
	.f = cmd_show_lru_parsed,
	.data = NULL,
	.help_str = "help: show lru flow",
	.tokens = {
		(void *)&cmd_show_lru_help,
		NULL,
	},
};
